...
def version():
    ...
    history = {
        ...
        0.17: "поиск по всем задачам при вводе текста, правка мелких багов",
        0.18: "- роутинг (подстановка урл и перенаправление на нужный компонент при обновлении страницы)",
        0.19: "- подсветка текущего пункта меню<br>" +
              "- правка невозможности экспорта задач, если соответствие установлено только с эво",
        0.22: "- уведомление о незавершенных задачах при экспорте<br>" +
        ...
        1.1:  "- стилизованы alert'ы<br>" +
              "- поправлены некоторые баги<br>" +
              "- добавлены новые",
        ...
        1.4:  "- фикс работы автокомплита при добавлении нескольких задч подряд<br>" +
              "- исправлен баг с добавлением нескольких параллельных задач",
        1.5:  "- настройки формата выгрузки задач в evo в меню 'настройки'",
        1.6:  "- небольшие правки авторизации, придется опять обновлять страницу, да. Сорян =\\",
        1.7:  "- разрешено редактирование названия задачи при экспорте (на случай, если вы его выгружаете в эво)<br>" +
              "- фикс добавления/редактирования одноименных задач в разных проектах<br>" +
              "- мелкие правки верстки",
        1.8:  "- фикс бага с выбором диапазона дат в другом месяце (история, экспорт задач)",
        1.9:  "- фикс бага с экспортом задач",
        1.10: "- фикс горизонтального скролла",
        1.11: "- кнопка обновления списка на странице истории",
        1.12: "- добавлена поддержка нового поля 'Номер задачи' в эво:<br><br>" +
        ...
        1.13: "- добавлен makefile с командами для сборки фронта и управления сервером<br>" +
              "- проведена оптимизация dockerfile, размер сборки снижен с 380мб до 90мб<br>" +
              "- исправлен баг, при регистрации нового пользователя тип базовых трекеров распознавался по названию<br>" +
              "- исправлены некоторые опечатки в хэлпе<br>" +
              "- исправлены баги, при сравнении строк вместо самих строк сравнивались их ссылки в памяти<br>" +
              "- исправлена синтаксическая ошибка в коде регистрации новых активностей<br>" +
              "- проведен рефакторинг зависимостей на беке, список приведен в порядок, стабилизирован, проставлены версии<br>" +
              "- зависимости и dockerfile обновлены до версии python 3.10, адаптирован код<br>" +
              "- в экспорт добавлен контроль суммарного времени по задаче между таймтрекером и внешним трекером<br><br>" +
              "-- теперь выгрузка будет блокироваться, если на трекере уже зарегистрировано столько же или больше времени, чем в таймтрекере. Точность сравнения пока ограничена 0.05ч<br>" +
              "-- работает это только для активностей с номером задачи<br>" +
              "- в экспорт добавлено обнаружение активностей на внешних трекерах, не зарегистрированных на таймтрекере<br><br>" +
              "-- выгрузка по задаче будет блокироваться, если на трекере по текущей задаче есть лишние часы<br>" +
              "-- работает это только для активностей с номером задачи<br>" +
              "-- пока лишние часы нужно будет регистрировать на таймтрекере вручную<br>" +
              "-- есть потенциал для внедрения импорта времени с внешних трекеров<br>" +
              "- в экспорт добавлен контроль состояния выгрузок<br><br>" +
              "-- теперь история экспортов сохраняется, для выявления уже экспортированных активностей, по каждому трекеру отдельно<br>" +
              "-- теперь выгрузка ранее выгруженной активности будет блокироваться<br>" +
              "- исправлен баг, апи-клиенте evo при запросе активностей игнорировал новое поле task_id<br>",
        1.14: "- корректный подсчет суммы часов при экспорте записей за несколько дней, обновление сборки фронта<br>",
    }

    current_version = 1.14  # подобное не работает на боевом нормально - list(history.keys())[-1]
