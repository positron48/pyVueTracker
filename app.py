...
def version():
    ...
    history = {
        ...
        2.02: "- доработка массовой отметки задач в экспорте по трекеру, если экспортируется несколько дней сразу<br>"
              "- уменьшена область срабатывания чекбоксов массовой отметки задач в экспорте по трекеру",
    }

    current_version = 2.02  # подобное не работает на боевом нормально - list(history.keys())[-1]
