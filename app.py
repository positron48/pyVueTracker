...
def version():
    ...
    # todo: вынести в отдельный файлик, обновлять автоматом
    ...
    history = {
        ...
        0.13: "увеличен интервал проверки обновлений до 5 минут fix",
        ...
        1.1:  "- стилизованы alert'ы<br>" +
              ...
              "- добавлены новые",
        1.100:  "- активность экспорта, только если есть задачи для выгрузки<br>" +
              "- правки верстки",
        1.101:  "- в экспорте задач в качестве разделителя в часах можно использовать как запятую, так и точку",
        1.102:  "- фикс работы автокомплита при добавлении нескольких задч подряд<br>" +
              "- исправлен баг с добавлением нескольких параллельных задач",
        1.103:  "- настройки формата выгрузки задач в evo в меню 'настройки'",
        1.104:  "- небольшие правки авторизации, придется опять обновлять страницу, да. Сорян =\\",
        1.105:  "- разрешено редактирование названия задачи при экспорте (на случай, если вы его выгружаете в эво)<br>" +
              "- фикс добавления/редактирования одноименных задач в разных проектах<br>" +
              "- мелкие правки верстки",
        1.106:  "- фикс бага с выбором диапазона дат в другом месяце (история, экспорт задач)",
        1.107:  "- фикс бага с экспортом задач",
        1.108: "- фикс горизонтального скролла",
        1.109: "- кнопка обновления списка на странице истории",
        1.93: "- добавлена поддержка нового поля 'Номер задачи' в эво:<br><br>" +
        ...
        2.01: "- добавлена возможность массовой отметки задач в экспорте по трекеру",
        2.03: "- доработка массовой отметки задач в экспорте по трекеру, если экспортируется несколько дней сразу<br>" +
              "- уменьшена область срабатывания чекбоксов массовой отметки задач в экспорте по трекеру",
        2.04: "- трансляция ошибок из эво в таймтрекер при выгрузке задач",
        2.05: "- фикс багов",
        2.10: "<b> Поддержка трекера jira </b><br><br>" +
              "Настройки > Добавить JIRA<br>" +
              "Нужно указать url jira (без слеша на конце) и затем авторизоваться, введя логин/пароль<br>" +
              "В задаче в редмайне в поле 'Задача в системе заказчика' должна быть указана ссылка на задачу в jira.<br><br>" +
              "Если домен в сссылке совпадает с указанным в трекере - по задаче будут получены данные и появится возможность " +
              "выгрузки затрат в jira (модуль Tempo).",
        2.11: "- убрана подстановка лишних пробелов при выборе задачи из списка",
        2.12: "- фикс верстки для корректного выделения номера задачи в списке задач дабл кликом",
        2.13: "- корректный подсчет суммы часов при экспорте записей за несколько дней, обновление сборки фронта<br>",
    }

    # получение последней версии из истории
    items = list(history.keys())
    last_key = items[-1]
    current_version = last_key
